    ...
    return (
        <div>
            <ResponsiveContainer width="100%" height={150}>
                <LineChart width={350} height={150} data={aggregated}
                    margin={{top: 20, right: 30, left: 0, bottom: 0}}>
                <YAxis
                    label={{value:"kg", position: 'top', offset: 10, fontSize: "12px"}}
                    domain={[minWeight - 5, maxWeight + 5]}
                    tick={{fontSize: '12px'}}
                    width={40}
                />
                <Line type='linear' dataKey='y' stroke='#E38B21' fill='#E38B21' dot={{strokeWidth: 2, r: 1}} />
                </LineChart>
            </ResponsiveContainer>
        </div>
    );
