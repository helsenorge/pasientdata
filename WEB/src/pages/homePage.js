...
class homePage extends Component {
    ...
    responseGoogle(response){
        axios.get(API + DEFAULT_QUERY, { headers: { Authorization: "Bearer " + response.accessToken } })
        .then((res) => {
            this.setState({ 
    ...
    render() {
        const list = this.state.dataSet.map((item, index) => <li key ={index} > {index} {item.startTimeNanos} - {item.endTimeNanos} : {item.value[0].intVal}</li>);
        return (
        <div>
        {
            <GoogleLogin
